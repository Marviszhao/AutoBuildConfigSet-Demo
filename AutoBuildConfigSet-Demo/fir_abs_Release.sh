#! bin/bash
#Author:Bruce http://blog.csdn.net/wang631106979/article/details/52299083
#Update Date:2015.06.23
#Use:命令行进入工程目录直接执行sh fir_autobuild_script.sh即可完成打包发布到fir.im

export LC_ALL=zh_CN.GB2312;
export LANG=zh_CN.GB2312

###############设置需编译的项目配置名称
buildConfig="Release" #编译的方式有,Debug,Release，自定义的 DebugOn,DebugOnlineArch,Enterprise等
...
curl -X PUT --data "changelog=$changelog" http://fir.im/api/v2/app/appID?token=填上你们公司从fir.im官网上申请到的APIToken
echo "\n打包上传更新成功！"
rm -rf $buildAppToDir
echo "删除${buildAppToDir} 成功"
rm -rf $projectDir/tmp
echo "删除${projectDir}/tmp 成功"
